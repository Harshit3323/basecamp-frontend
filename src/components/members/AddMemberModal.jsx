...
import { useAddMember } from "../../hooks/queries/useMembers";

const addMemberSchema = z.object({
  email: z.string().min(1, "User email is required").email("Invalid email address"),
  role: z.enum(["admin", "project_admin", "member"]).default("member"),
});
...
export const AddMemberModal = ({ isOpen, onClose, projectId }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || "Failed to add member. Please verify user email.";
      setApiError(msg);
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative rounded-xl shadow-sm">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-700/70 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 text-sm transition-all"
              />
            </div>
            {errors.email && (
          ...
          <div>
            ...
            >
              <option value="member">Member (View & update subtask completion)</option>
              <option value="project_admin">Project Admin (Manage tasks & subtasks)</option>
              <option value="admin">Admin (Full project workspace control)</option>
            </select>
          </div>
